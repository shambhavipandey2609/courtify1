...
const LoginSignup = () => {
  ...
  const onSubmitHandler = async (event) => {
    ...
    if (state === "Sign Up") {
      ...
      requestBody.append("barCouncilId", barCouncilId);
      requestBody.append("areaOfPractice", areaOfPractice);
      requestBody.append("phoneNumber", phoneNumber);
      requestBody.append("yearsOfExperience", yearsOfExperience);
      if (photo) requestBody.append("photo", photo);
    } else {
    ...
    try {
      const response = await fetch(
        ...
        {
          method: "POST",
          headers: state === "Login" ? headers : undefined,
          body: requestBody,
        }
      );
      ...
      if (state === "Sign Up") {
        ...
        navigate("/lawyer.html"); // Redirect after login
      }
    } catch (err) {
      setError(err.message);
    }
