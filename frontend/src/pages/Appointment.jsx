...
const Appointment = () => {
  const { lawyerId } = useParams();
  const { lawyers, currencySymbol, userData } = useContext(AppContext);

  //const { lawyers, currencySymbol, user } = useContext(AppContext);
  const daysOfWeek = ["SUN", "MON", "TUE", "WED", "THU", "FRI", "SAT"];
  ...
  //   try {
  //     const response = await axios.post("http://localhost:4001/api/consultations", {
  ...
  // };
  const bookAppointment = async () => {
    const dateObj = lawyerSlots[slotIndex][0].datetime;
    ...
    try {
      const response = await fetch("http://localhost:4001/api/consultations", {
        method: "POST",
        headers: {
 ...
 // 🔁 Replace with actual logged-in client
          date: slotDate,
          time: slotTime,
        }),
      });
